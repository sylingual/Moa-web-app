...
export default function App() {
  ...
  const t = T[lang];
  const revCount = data.cards.filter(c => c.status === "new" || c.status === "review" || c.status === "in_progress").length;
  const studiedCount = data.cards.filter(c => c.status === "studied").length;
  const acqCount = data.cards.filter(c => c.status === "acquired").length;
  const exerciseCards = useMemo(() => data.cards.filter(c => c.status === "studied" || c.status === "acquired"), [data.cards]);
        ...
        {view === "library" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto" }}>
            <div style={{ padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${C.border}` }}>
              <span style={{ fontSize: 12, color: C.txtM }}>{data.cards.length} {t.points} · {studiedCount} {t.statusStudied.toLowerCase()} · {acqCount} {t.statusAcquired.toLowerCase()}</span>
              {data.cards.length > 0 && (
