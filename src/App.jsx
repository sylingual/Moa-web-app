...
Return JSON with these exact fields:
{
  ...
  "profileInsights": {
    "interests": "Any new interests, hobbies, or preferences the student mentioned (artists, shows, foods, etc.). Empty string if nothing new.",
    "level": "Any observation about the student's actual level based on their performance (e.g. 'handles ~아/어서 naturally but hesitates with honorifics'). Empty string if no clear observation.",
    "notes": "Any other personal info shared (job, reason for learning, study habits, etc.). Empty string if nothing."
  }
}`;
...
export default function App() {
  ...
  const endLesson = async () => {
    ...
    try {
      ...
      // Mark card as acquired and save summary + updated profile
      const updatedCards = data.cards.map(c => c.korean === lCard.korean ? { ...c, status: "acquired" } : c);
      save({ ...data, cards: updatedCards, summaries: [...(data.summaries || []), summary], profile: currentProfile });
    } catch (e) {
      console.error(e);
                  ...
                  {lessonSummary && (
                    <div style={{ background: C.s2, border: `1px solid ${C.okB}`, borderRadius: 10, padding: 16, margin: "4px 0" }}>
                      ...
                      <button onClick={() => { setLCard(null); setConv([]); setLessonDone(false); setLessonSummary(null); setView("import"); }}
                        style={{ marginTop: 14, padding: "6px 16px", borderRadius: 6, background: C.acc, color: C.onAcc, border: "none", fontFamily: "'Plus Jakarta Sans'", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                        → {t.newLesson}
                      </button>
                    </div>
                  )}
