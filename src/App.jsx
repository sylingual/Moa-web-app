...
async function continueChat(card, conv, action, lang) {
  ...
  const correctByPhase = {
    ...
    CONSOLIDATION: `The student answered correctly! Briefly confirm (1 sentence). Now give a mini production exercise: provide a situation in ${L} and ask the student to write a Korean sentence using "${card.korean}". After they answer (or if this is the exercise itself), wrap up the lesson: summarize what was covered, congratulate the student, and explicitly tell them they can click the "End lesson" button to save their progress and see their summary.`,
  };
  ...
  const instruction = acts[action] || `The student said: "${action}". Respond naturally as a Socratic Korean teacher. Stay focused on the structure "${card.korean}". If they ask a question, answer it helpfully. If they attempt Korean, gently correct any mistakes. Always try to keep the lesson moving forward with the current phase in mind.`;
            ...
            <div style={{ flex: 1, display: "flex", overflow: "hidden", flexDirection: window.innerWidth < 700 ? "column" : "row" }}>
              <div style={{ width: window.innerWidth < 700 ? "100%" : "40%", maxHeight: window.innerWidth < 700 ? "35%" : "none", flexShrink: 0, borderRight: window.innerWidth >= 700 ? `1px solid ${C.border}` : "none", borderBottom: window.innerWidth < 700 ? `1px solid ${C.border}` : "none", display: "flex", flexDirection: "column", background: C.s2 }}>
                <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.border}`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  {conv.length >= 2 && !lessonDone && (
                    <button onClick={endLesson} disabled={lLoad}
                      style={{ padding: "5px 12px", borderRadius: 6, border: `1px solid ${C.okB}`, background: C.okBg, color: C.ok, fontFamily: "'Plus Jakarta Sans'", fontSize: 11, fontWeight: 500, cursor: lLoad ? "default" : "pointer", flexShrink: 0 }}>
                      ✓ {t.endLesson}
                    </button>
                  )}
                </div>
