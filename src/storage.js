...
// syncData: the main sync function called when connecting with a code
// Returns { ok: true, data, source: "remote"|"pushed" } or { ok: false, error: "..." }
export async function syncData(userId, localData) {
  ...
  if (remote.data) {
    // Remote data exists: use it, save locally
    ...
    const result = await saveToSupabase(userId, localData);
    if (!result.ok) return { ok: false, error: result.error };
    return { ok: true, data: localData, source: 'pushed' };
  }
}
