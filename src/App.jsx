...
import { loadData, saveData, isSupabaseConfigured, DEFAULT_DATA, DEFAULT_PROFILE } from "./storage.js";
...
// =============================================
async function analyzeText(text, existing, lang, context) {
  const known = existing.map((c) => c.korean).join(", ");
  const L = lang === "fr" ? "French" : "English";
  const sys = `You are an expert Korean language analyst. Your job is to find the most interesting and teachable grammar structures or expressions in a Korean text, personalized for this specific learner.
...
2. Prefer structures that are emotionally expressive, subjective, or commonly found in blogs and conversation (e.g. 어찌나...는지, ~아/어 가다, ~하는 모습, ~(으)ㄹ 뻔하다)
...
- "description_fr": one clear sentence in ${L === "French" ? "French" : "English"} explaining what it means and when to use it
- "description_en": same in ${L === "English" ? "English" : "the other language"}
...
async function startSocratic(card, article, lang, context) {
  const L = lang === "fr" ? "French" : "English";
  const d = lang === "fr" ? card.description_fr : card.description_en;
  const sys = `You are a warm, encouraging Korean language teacher who uses the Socratic method. You NEVER explain a rule directly. Instead, you guide the student to discover it themselves through observation and pattern recognition.
...
2. Then show 2 NEW example sentences (not from the article) that use the same structure in different contexts. If you know the student's interests (see LEARNER PROFILE above), draw examples from those topics (e.g. if they like BTS, use a sentence a fan might write; if they like cooking, use a recipe blog context). Choose examples where the meaning of the structure becomes obvious from context.
...
PERSONALIZATION:
- If the learner has studied related structures before (see LEARNING HISTORY), you can reference them: "Tu te souviens de ~아/어 가다 ? Celui-ci est un peu different..."
...
async function continueChat(card, conv, action, lang) {
  ...
  const acts = {
    ...
    "incorrect, explain": `The student answered incorrectly. Do NOT just say "wrong" and do NOT give the correct answer directly. Instead:
1. Gently say it is not quite right (be encouraging)
2. Identify specifically what went wrong (wrong particle? wrong conjugation? wrong nuance?)
3. Give a targeted hint that helps them find the answer themselves (e.g. "look at the verb ending", "think about whether this is an adjective or a verb", "remember how ~지 attaches to the stem")
4. Ask them to try again with this hint
Do NOT show the corrected sentence. Let them figure it out. Use "label" as the key name for MCQ options if you include any.`
  };
...
CRITICAL CORRECTION RULE: If the student attempts to write Korean and makes mistakes, do NOT give them the corrected sentence. Instead:
1. Acknowledge their effort positively
2. Point out specifically what needs fixing (e.g. "the particle after 서비스 needs to change" or "check how you conjugated the verb")
...
async function genExercise(cards, mode, lang, context) {
  const L = lang === "fr" ? "French" : "English";
...
Start with the easiest structure and increase difficulty. Present all 3 in your message.`
  ...
  const sys = `You are a Korean language exercise designer. Speak in ${L}. Be clear and encouraging.
      ...
      <div key={card.id}>
        <div style={{ display: "flex", alignItems: "stretch", marginLeft: depth * 28 }}>
          ...
          <button onClick={() => onReview(card)}
            ...
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: rev ? C.warn : C.ok, flexShrink: 0 }} />
            ...
            <button onClick={e => { e.stopPropagation(); onToggle(card.id); }}
              style={{ fontSize: 10, padding: "2px 6px", borderRadius: 8, border: "none", background: rev ? C.warnBg : C.okBg, color: rev ? C.warn : C.ok, cursor: "pointer", fontFamily: "'Plus Jakarta Sans'", flexShrink: 0 }}>
              {rev ? t.review : t.acquired}
            </button>
          </button>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved && saved.card && saved.conv && saved.conv.length > 0) {
        setLCard(saved.card);
        setLArticle(saved.article || "");
        // Restore conv without callbacks (onSelect will be re-attached in render)
  ...
  }, [conv, lCard, lArticle, lessonDone, lessonSummary, lessonRestored]);
  ...
  const doAnalyze = async () => {
    ...
    setImpStep("scanning");
    try {
      setFound(await analyzeText(impText, data.cards, lang, context));
  ...
  const beginLesson = async (point, art) => {
    ...
    try { const r = await startSocratic(card, text, lang, context); setConv([{ role: "ai", content: r.message, options: r.options, selected: null }]); }
  ...
  const endLesson = async () => {
    ...
    try {
      ...
      // Mark card as acquired and save summary + derived cards + updated profile
      const updatedCards = data.cards.map(c => c.korean === lCard.korean ? { ...c, status: "acquired" } : c);
  ...
  const launchEx = async () => {
    ...
    try { const r = await genExercise(sel, exMode, lang, context); setExConv([{ role: "ai", content: r.message, options: r.options || null, selected: null }]); }
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto" }}>
            ...
            {data.cards.length === 0
              ...
              : libView === "grid"
                ...
                : <TreeView cards={data.cards} t={t} onToggle={(id) => toggleSt(id)} onReview={(c) => reviewCard(c)} />
            }
          </div>
        )}
            ...
            {impStep === "picks" && (
              <div style={{ width: "100%", maxWidth: 520, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                  ...
                  <button onClick={async () => { setImpStep("scanning"); try { const m = await analyzeText(impText, [...data.cards, ...found.map(p => ({ korean: p.korean }))], lang, context); setFound([...found, ...m]); } catch (e) { console.error(e); } setImpStep("picks"); }}
