    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans'", display: "flex", flexDirection: "column", height: "100%", background: C.s0 }}>
      ...
      <header style={{ display: "flex", alignItems: "stretch", padding: "0 12px", height: 46, background: C.s2, borderBottom: `1px solid ${C.border}`, flexShrink: 0, overflowX: "auto" }}>
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", marginRight: 12, flexShrink: 0 }}>
          ...
          {tlOpen && (
            <div onClick={e => e.stopPropagation()} style={{ position: "absolute", left: 0, top: "100%", marginTop: 4, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden", zIndex: 50, minWidth: 160, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
              {enabledTLs.map(code => (
                <button key={code} onClick={() => switchTargetLang(code)}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", fontSize: 12, color: tl === code ? C.acc : C.txtS, fontWeight: tl === code ? 500 : 400, cursor: "pointer", border: "none", background: tl === code ? C.accBg : "none", width: "100%", fontFamily: "'Plus Jakarta Sans'", textAlign: "left" }}>
              ...
              {Object.keys(TARGET_LANGS).filter(code => !enabledTLs.includes(code)).length > 0 && (
                <div style={{ borderTop: `1px solid ${C.border}` }}>
                  {Object.keys(TARGET_LANGS).filter(code => !enabledTLs.includes(code)).map(code => (
                    <button key={code} onClick={() => addTargetLang(code)}
                      style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", fontSize: 12, color: C.txtM, cursor: "pointer", border: "none", background: "none", width: "100%", fontFamily: "'Plus Jakarta Sans'", textAlign: "left" }}>
                      + {TARGET_LANGS[code]?.flag} {getTargetLangName(code, lang)}
                    </button>
                  ))}
                </div>
