          ...
          {syncId ? (
            ...
            <>
              ...
              <button onClick={handleSync} disabled={syncStatus === "loading" || !syncInput.trim()}
                style={{ padding: "5px 12px", borderRadius: 6, background: syncInput.trim() && syncStatus !== "loading" ? C.acc : C.s1, color: syncInput.trim() && syncStatus !== "loading" ? C.onAcc : C.txtM, border: "none", fontSize: 12, fontFamily: "'Plus Jakarta Sans'", cursor: syncInput.trim() && syncStatus !== "loading" ? "pointer" : "default" }}>
                {syncStatus === "loading" ? t.syncLoading : t.connect}
              </button>
              {syncStatus === "error" && <span style={{ fontSize: 11, color: C.warn }}>{t.syncError}</span>}
              {syncStatus === "success" && <span style={{ fontSize: 11, color: C.ok }}>✓ {t.syncSuccess}</span>}
              {!syncStatus && <span style={{ fontSize: 11, color: C.txtM, flex: 1 }}>{t.syncInfo}</span>}
            </>
          )}
        </div>
      )}
