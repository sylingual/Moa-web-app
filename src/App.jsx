...
function GrammarCard({ card, t, onToggle, onReview }) {
  ...
  return (
    <div onClick={onReview}
      ...
      <div style={{ position: "absolute", bottom: -12, right: -3, fontFamily: tFont, fontSize: 72, fontWeight: 500, color: C.acc, opacity: 0.07, lineHeight: 1, pointerEvents: "none", userSelect: "none" }}>
      ...
      <div style={{ fontFamily: tFont, fontSize: 17, color: C.txt, marginBottom: 3 }}>{card.korean}</div>
      ...
      <div style={{ background: C.s1, borderRadius: 6, padding: "7px 9px", marginBottom: 8 }}>
        <div style={{ fontFamily: tFont, fontSize: 12.5, color: C.txt }}>{card.example_kr}</div>
      ...
      {card.parentKorean && (
        <div style={{ fontSize: 10, color: C.acc, marginBottom: 6, display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ opacity: 0.6 }}>↳</span> {t.derivedFrom} <span style={{ fontFamily: tFont, fontWeight: 500 }}>{card.parentKorean}</span>
        </div>
      )}
  ...
  return (
    <div style={{ background: C.s2, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
      <button onClick={() => setOpen(!open)}
        ...
        <span style={{ fontFamily: tFont, fontSize: 14, color: C.txt, fontWeight: 500 }}>{summary.cardKorean}</span>
      ...
      <div key={card.id}>
        <div style={{ display: "flex", alignItems: "stretch", marginLeft: depth * 28 }}>
          ...
          <button onClick={() => onReview(card)}
            ...
            <span style={{ fontFamily: tFont, fontSize: 14, color: C.txt, fontWeight: 500 }}>{card.korean}</span>
            ...
            <div style={{ flex: 1, display: "flex", overflow: "hidden", flexDirection: window.innerWidth < 700 ? "column" : "row" }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", background: C.s1, minHeight: 0 }}>
                <div ref={msgsR} style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {/* LESSON SUMMARY */}
                  {lessonSummary && (
                    <div style={{ background: C.s2, border: `1px solid ${lessonSummary.error ? C.warnB : C.okB}`, borderRadius: 10, padding: 16, margin: "4px 0" }}>
                      ...
                      {lessonSummary.error ? (
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: C.txtM, lineHeight: 1.5, marginBottom: 12, background: C.s1, padding: "6px 10px", borderRadius: 6, fontFamily: "monospace", wordBreak: "break-all" }}>
                            {lessonSummary.error.substring(0, 200)}
                          </div>
