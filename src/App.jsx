...
Apply this to EVERY translation you write, including translations inside MCQ options if the option is itself a translation. Do NOT wrap explanations, grammar notes, or questions: only actual translations of target-language text.`;

// =============================================
// AI CALL (via serverless proxy)
// =============================================
async function callAI(systemPrompt, userMessage, maxTokens, useSearch) {
...
Be explicit that these are search suggestions, not direct links.`;
  ...
  try {
    return await callAI(sys, `Find pedagogical resources explaining the ${TL} grammar structure: ${card.korean}`, 1600, true);
  } catch (e) {
    if (String(e.message).includes("SEARCH_QUOTA")) {
      const r = await callAI(fallbackSys, `Suggest how to find resources for: ${card.korean}`, 1200, false);
      return { text: r.text, sources: [], degraded: true };
    }
    throw e;
  }
}
...
Finish with 2-3 precise search queries the student can run themselves to find authentic occurrences.`;
  ...
  try {
    return await callAI(sys, `Find authentic real-world examples of the ${TL} structure: ${card.korean}`, 2000, true);
  } catch (e) {
    if (String(e.message).includes("SEARCH_QUOTA")) {
      const r = await callAI(fallbackSys, `Give varied register examples for: ${card.korean}`, 1600, false);
      return { text: r.text, sources: [], degraded: true };
    }
    throw e;
  }
...
async function continueExercise(cards, mode, lang, conv, wasCorrect, tlCode) {
  ...
  const next = isLast
    ? `This was the last question. After your feedback, wrap up: give a short encouraging summary of how the session went (2-3 sentences) and tell the student they can start a new exercise from the Exercise tab. Do NOT ask another question and do NOT include options.`
    : mode === "qcm"
            ...
            {msg.options.map((o, i) => {
              ...
              return (
                <button key={i}
                  ...
                  {renderMarkdown(oLabel, revealAll)}
                </button>
              );
            })}
            {canPick && preSel && (
          ...
          showRecap && recapCard ? (
            ...
            <div style={{ flex: 1, overflowY: "auto", display: "flex", justifyContent: "center" }}>
              <div style={{ width: "100%", maxWidth: 540, padding: "24px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8 }}>
                    {[
                      ...
                      <button key={a.k} onClick={() => startRecapAction(a.k)}
                        style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 13px", borderRadius: 10, border: `1px solid ${recapMode === a.k ? C.acc : C.border}`, background: recapMode === a.k ? C.accBg : C.s2, cursor: "pointer", fontFamily: "'Plus Jakarta Sans'", fontSize: 12.5, color: recapMode === a.k ? C.acc : C.txt, fontWeight: recapMode === a.k ? 500 : 400, textAlign: "left" }}>
                ...
                {/* Quick practice chat */}
                {recapMode && (
                  <div style={{ border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", background: C.s1 }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: 12, maxHeight: 420, overflowY: "auto" }}>
                      {recapConv.map((m, i) => (
                        <Bubble key={i} revealAll={revealTr}
                    ...
                    <div style={{ padding: "8px 10px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 6, background: C.s2, alignItems: "center" }}>
                      ...
                      <button onClick={() => { setRecapMode(null); setRecapConv([]); }} title={t.backToRecap}
                        style={{ padding: "0 10px", height: 30, border: `1px solid ${C.border}`, borderRadius: 6, background: C.s2, cursor: "pointer", color: C.txtS, fontSize: 11, fontFamily: "'Plus Jakarta Sans'", flexShrink: 0 }}>✕</button>
                    </div>
                  </div>
                )}
