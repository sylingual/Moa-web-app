...
function AppInner() {
  ...
  const filteredCards = useMemo(
    () => (tl ? allCards.filter(c => (c.targetLang || "ko") === tl) : allCards),
    [allCards, tl]
  );
    ...
    if (lCard && conv.length > 0) {
      try {
        localStorage.setItem("moa-active-lesson", JSON.stringify({
          ...
          conv: conv.map(m => ({ role: m.role, content: m.content, options: m.options || null, selected: m.selected || null, sources: m.sources || null })),
  ...
  const quickAct = async (a) => {
    ...
    try {
      if (a === "resources" || a === "realExamples") {
        ...
        setSearching(false);
        setConv([...u, { role: "ai", content: r.text, sources: r.sources, options: null, selected: null }]);
      } else {
